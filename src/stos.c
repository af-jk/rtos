...
// Would it be best to put these within some sort of kernel struct to make it
// more compact?
static stos_list_t stos_ready_list;    // ready tasks
static stos_list_t stos_blocked_list;  // blocked tasks

/* Vision
 * stos_ready_list - tasks which are ready (sorted by priority - DOES NOT
 * CONTAIN THE ACTIVE TASK!) stos_block_list - tasks which are blocked (sorted
 * by timeout) stos_active     - task which is currently TASK_ACTIVE (should
 * only ever have one task in this state!) stos_next       - task which is going
 * to be scheduled next:
 *                  1. Will be stos_active if ready list doesnt have any task
...
void STOS_Init(void (*handler)(void), uint32_t size) {
    ...
    // Add idle task
    STOS_CreateTask(&stos_idle_tcb, handler, STOS_IDLE_DEFAULT_PRIORITY, size);
    ...
    stos_active =
        STOS_LIST_CONTAINER_GET_TCB(stos_ready_list.next, stos_tcb_t, list);
...
void STOS_CreateTask(stos_tcb_t *task, void (*handler)(void), uint32_t pri,
                     uint32_t size) {
    uint32_t *psp;
    __asm volatile(" MRS   %[psp_var], psp    \n" : [psp_var] "=r"(psp) : :);
    uint32_t *init_sp = (uint32_t *)((uint32_t)psp);

    // When the task first starts, we're going to pre-fill the stack frame as
    // such PC and LR are required, the others are filled in just for easier
    // debugging
    *(--init_sp) = (1U << 24);         // xPSR
...
void STOS_AddTask(stos_tcb_t *task, stos_list_t *task_list) {
    ...
    if (task_list == &stos_ready_list) {
        for (run = task_list->next; run != task_list; run = run->next) {
            stos_tcb_t *run_tcb =
                STOS_LIST_CONTAINER_GET_TCB(run, stos_tcb_t, list);
    ...
    if (task_list == &stos_blocked_list) {
        ...
        for (run = task_list->next; run != task_list; run = run->next) {
            stos_tcb_t *run_tcb = STOS_LIST_CONTAINER_GET_TCB(run, stos_tcb_t,
    list); if (task->pri > run_tcb->pri) { idx = run_tcb; break;
            }
        }
...
void STOS_Schedule() {
    // The head of the stos_ready_list will be the task with the highest
    // priority that has the longest without being scheduled. This will be
    // maintained by always adding tasks to be at the end of their respected
    // ordered priority section. Ex: If i have three tasks w/ priority 8, and my
    // active task has a priority of 8, when I perform this operation I will pop
    // my top task, and push the active one to the end of the tasks with
    // priority 8.
    stos_tcb_t *ready_task_head =
        STOS_LIST_CONTAINER_GET_TCB(stos_ready_list.next, stos_tcb_t, list);

    if (ready_task_head->pri < stos_active->pri)
        return;  // never call pend_sv because we don't need to switch tasks

    // Only get here if ready_task_head >= stos_active which means we want to
    // switch tasks
    ...
    // Put the currently active task back into the ready task list so that we
    // can schedule it in the future
...
void sys_tick_handler(void) { STOS_Schedule(); }
...
__attribute__((naked)) void pend_sv_handler(void) {
    __asm volatile(
        ...
        // stack xPSR, RA, LR, R12, R3-R0 and will update the PSP we need to
        // further push registers r11-r4 and then update the stos_active->sp
        ...
        // Now that I'm done I need to maintain r1 and set stos_active =
        // stos_next
        " LDR   r3, =stos_active    \n"
        " STR   r2, [r3, #0x00]     \n"
